...
public class MyProtobufIDLProxy {
        ...
        for (EnumElement enumType : enums) {
            ...
            if (!generateSouceOnly) {
                Class<?> newClass = JDKCompilerHelper.getJdkCompiler().compile(codeDependent.getClassName(),
                        codeDependent.code, MyProtobufIDLProxy.class.getClassLoader(), null, -1);
                ret.add(newClass);
            } else {
     ...
     * @param mappedUniName the mapped uni name
     * @param isUniName the is uni name
     * @param packageMapping 包名映射解决字段泛型类型错误
     * @return the list
     */
    private static List<Class<?>> createMessageClass(ProtoFile protoFile, boolean multi, boolean debug,
                                                     ...
                                                     Set<String> enumNames, Set<String> packages, Map<String, String> mappedUniName, boolean isUniName, Map<String, String> packageMapping) {
        ...
        for (MessageElement mt : messageTypes) {
            ...
            cd = createCodeByType(protoFile, (MessageElement) mt, enumNames, true, new ArrayList<TypeElement>(), cds,
                    packages, mappedUniName, isUniName,packageMapping);
        ...
        // copy cds
        List<CodeDependent> copiedCds = new ArrayList<CodeDependent>(cds);
    ...
    private static CodeDependent createCodeByType(
            ProtoFile protoFile, MessageElement type, Set<String> enumNames,
            boolean topLevelClass, List<TypeElement> parentNestedTypes, List<CodeDependent> cds, Set<String> packages,
            Map<String, String> mappedUniName, boolean isUniName
            ...
            if (fType == null) {
                ...
                if (typeName.equals(ProtobufIDLGenerator.GOOGLE_PROTOBUF_ANY_DEF)) {
                    ...
                    if (!isNestedTypeDependency(javaType, checkNestedTypes, mappedUniName, isUniName)) {
                        ...
                        String javaTypeMapping = packageMapping.get(javaType);

                        if(javaTypeMapping!=null){
                            javaType = javaTypeMapping+"." + javaType;
                        }else {
                            if (packages.size() > 0) {
                                String pkg = packages.iterator().next();
                                if (pkg != null) {
    ...
    private static Map<String, IDLProxyObject> doCreatePro(File file, boolean multi, boolean debug, File path,
        ...
        Map<String, IDLProxyObject> stringIDLProxyObjectMap = doCreatePro(protoFiles, multi, debug, path, generateSouceOnly, sourceOutputDir, cds, compiledClass,
                mappedUniName, isUniName);
        return stringIDLProxyObjectMap;

    }

    //(file, true, false, null, true, sourceOutputPath, cds, compiledClass, new HashMap<String, String>(),
    public static Map<String, IDLProxyObject> createAll(File input,File Dir,File outPut) throws IOException {
        return doCreateProAll(input,true,true,null,true,outPut,
                new ArrayList<CodeDependent>(),new HashSet<String>(),new HashMap<String, String>(),false,Dir);
    }

    /**
     *
     * @param file proto文件目录
     * @param multi
     * @param debug
     * @param path
     ...
     * @throws IOException
     */
    public static Map<String, IDLProxyObject> doCreateProAll(File file, boolean multi, boolean debug, File path,
                                                             boolean generateSouceOnly, File sourceOutputDir, List<CodeDependent> cds, Set<String> compiledClass,
                                                             Map<String, String> mappedUniName, boolean isUniName,File dir) throws IOException {
        checkDirectory(generateSouceOnly, sourceOutputDir);

        // to find all PROTO file if using import command
        //List<ProtoFile> protoFiles = findRelateProtoFiles(file, new HashSet<String>());
        Set<ProtoFile> protoFilesSet = new HashSet<>();
        List<File> protoList = new ArrayList<>();
        if(dir!=null){
            getAllProtoFile(dir,protoList);
        }else{
            protoFilesSet.addAll(findRelateProtoFiles(file, new HashSet<String>()));
        }

        protoList.forEach(i->{
            try {
                protoFilesSet.addAll(findRelateProtoFiles(i, new HashSet<String>()));
            } catch (IOException e) {
                e.printStackTrace();
            }
        });
        List<ProtoFile> protoFiles = new ArrayList<>(protoFilesSet);
    ...
    private static void getAllProtoFile(File dir,List<File> list){
        if(dir.isFile()){
            if(dir.getName().endsWith(".proto")){
                list.add(dir);
            }
        }else {
            Arrays.stream(dir.listFiles()).forEach(i->getAllProtoFile(i,list));
        }
    }
        ...
        for (ProtoFile protoFile : protoFiles) {
            ...
            List<Class<?>> messageClasses =
                    createMessageClass(protoFile, multi, debug, generateSouceOnly, sourceOutputDir, cds, compiledClass,
                            new HashSet<String>(enumTypes.keySet()), packages, mappedUniName, isUniName,packageMapping);
    ...
    private static String getProxyClassName(String name, Set<String> pkgs, Map<String, String> mappedUniName,
        ...
        if (name.indexOf(PACKAGE_SPLIT_CHAR) != -1) {
            ...
            if (uniName == null) {
                ...
                final String className = NameMappingUtil.mapClassName(uniName);
                //mappedUniName.put(clsName, className);
